...
import Web3HttpProvider from 'web3-providers-http'; // Contract Addresses

const WEB3ANALYTICS_ADDRESS = '0x6f521f66D4B8f223326194B8580230CC80d21A2F';
const WEB3ANALYTICS_PAYMASTER_ADDRESS = '0xc4a9Dd8785B50Ff51E7FAd38950D7Bb039fE21ab'; // Set up Ceramic
...
export default function web3Analytics(userConfig) {
  ...
  let q_ = Promise.resolve(); // `seed` must be a 32-byte long Uint8Array
  ...
  async function authenticateCeramic(seed) {
    ...
    console.log(did); // Authenticate the DID with the provider

    await did.authenticate();
    console.log(did); // The Ceramic client can create and update streams using the authenticated DID
  ...
  async function checkAppRegistration() {
    ...
    const contract = await new ethers.Contract(WEB3ANALYTICS_ADDRESS, Web3AnalyticsABI, provider);
    return contract.isAppRegistered(appId);
  }

  async function registerUser(privateKey, did) {
    console.log(privateKey);
    console.log(did); // OpenGSN config
    ...
    const contract = await new ethers.Contract(WEB3ANALYTICS_ADDRESS, Web3AnalyticsABI, provider.getSigner(signer.address, signer.privateKey)); // Check if user is already registered
    ...
    console.log(`Registering user Address: ${signer.address} did: ${did}`); // If user is not registered, process now
  ...
  async function sendEvent(payload, authenticatedDID) {
    ...
    console.log(payload); // Create Event in Ceramic

    const [doc, eventsList] = await Promise.all([model.createTile('Event', payload), dataStore.get('events')]); // Make ID of Event object part of the object itself

    const content = doc.content;
    content.id = doc.id.toString();
    await doc.update(content); // Add Event object to Events index

    const events = eventsList?.events ?? [];
    await dataStore.set('events', {
      events: [...events, {
        id: doc.id.toUrl(),
        updated_at: payload.meta.ts
      }]
  ...
  return {
    ...
    initialize: async ({
      ...
      const privateKey = "0x" + u8a.toString(seed, 'base16'); // Authenticate Ceramic
      ...
      console.log(`App is Registered: ${appId}`); // Check event count

      const newEvents = await dataStore.get('events');
      console.log(newEvents); // attempt to register user on blockchain
